...
function generate(node) {
    ...
    const context = {
        ...
        //换行的时候 应该保留缩进，所以我们还要追加currentIndent*2个空格字符
        newline() {
        ...
        //取消缩进，即让currentIndent自增后，调用换行函数
        deIndent() {
            context.currentIndent--
            context.newline()
        }
...
function genFunctionDecl(node, context) {
    ...
    //2.node.id是一个标识符，用来描述函数的名称，即node.id.name
